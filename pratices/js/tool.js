...
function move (node,obj,method){
	clearInterval(node.tag);
	//alert(000)
	node.tag = setInterval(function(){
		//alert(111)
		var isTrue = true;
		for(var i in obj){
			var current = 0;
			if(i == "opacity"){
				current = parseInt(parseFloat(getStyle(node,i) * 100));
			}else{
				current = parseInt(getStyle(node,i));
			}
			var speed = (obj[i] - current) / 8;
			speed = speed > 0 ? Math.ceil(speed) : Math.floor(speed);
			if(current != obj[i]){
				isTrue = false;
			}
			if(i == "opacity"){
				node.style.filter = "alpha(opacity = " + (current + speed) + ")";
				node.style.opacity = (current + speed) / 100;
			}else{
				node.style[i] = current + speed + "px";
			}
